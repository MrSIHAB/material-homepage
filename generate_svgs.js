...
function replaceColors(svgText) {
    ...
    svgText = svgText.replace(
        /fill\s*=\s*"(?!none|currentColor|transparent)([^\"]*)"/gi,
        'fill="#000000"',
    );
    svgText = svgText.replace(
        /fill\s*=\s*'(?!none|currentColor|transparent)([^\']*)'/gi,
    ...
    svgText = svgText.replace(/stroke\s*=\s*"([^\"]*)"/gi, 'stroke="#000000"');
    svgText = svgText.replace(/stroke\s*=\s*'([^\']*)'/gi, "stroke='#000000'");
...
function main() {
    ...
    let out = header;
    entries.forEach((e) => {
        out += `const ${e.name} = ` + "`" + `${e.svg}` + "`" + `;\n\n`;
    });

    // Optionally export all
    out += `module.exports = {` + entries.map((e) => ` ${e.name}`).join(",") +
        ` };\n`;
